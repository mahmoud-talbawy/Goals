...
class GoalsAppSupabaseSupabaseUser extends BaseAuthUser {
  GoalsAppSupabaseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> goalsAppSupabaseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GoalsAppSupabaseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
